                ...
                <table class="table" id="">
                    <thead>
                        <tr>
                            ...
                            <th class="wd-10p py-2 tx-20">العنوان</th>
                    ...
                    <tbody class="tx-18">
                        ...
                        <tr>
                            ...
                            <td>
                                @if ($customer->picture)
                                    <img src="{{ URL::asset($customer->picture) }}" class="img-thumbnail rounded h-25 w-25">
                                @else
                            ...
                            <td>
                                <div class="d-flex flex-row justify-content-center btn-icon-list mx-5">
                                    @can('صورة العميل')
                                    <button class="btn btn-warning btn-icon" data-toggle="modal" data-target="#pictureModel" wire:click.prevent='editPicture({{ $customer->id }})' ><i class="typcn typcn-camera"></i></button>
                                    @endcan
                                    @can('تعديل العميل')
                                    <button class="btn btn-success btn-icon" data-toggle="modal" data-target="#updateModal" wire:click.prevent='edit({{ $customer->id }})'  ><i class="typcn typcn-edit"></i></button>
                                    @endcan
                                    @can('حذف العميل')
                                    <button class="btn btn-danger btn-icon" data-toggle="modal" data-target="#deleteModal" wire:click.prevent='delete({{ $customer->id }})'><i class="typcn typcn-trash"></i></button>
                                    @endcan
                                    @can('حالة العميل')
                                    <button class="btn btn-info btn-icon" data-toggle="modal" data-target="#statusModel" wire:click.prevent='status({{ $customer->id }})'><i class="fa fa-check"></i></button>
                                    @endcan
