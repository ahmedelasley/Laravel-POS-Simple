                ...
                <table class="table" id="">
                    ...
                    <tbody class="tx-18">
                        ...
                        <tr>
                            ...
                            <td>
                                @if ($supplier->picture)
                                    <img src="{{ URL::asset($supplier->picture) }}" class="img-thumbnail rounded h-25 w-25">
                                @else
                            ...
                            <td>
                                <div class="d-flex flex-row justify-content-center btn-icon-list mx-5">
                                    @can('صورة المورد')
                                    <button class="btn btn-warning btn-icon" data-toggle="modal" data-target="#pictureModel" wire:click.prevent='editPicture({{ $supplier->id }})' ><i class="typcn typcn-camera"></i></button>
                                    @endcan
                                    @can('تعديل المورد')
                                    <button class="btn btn-success btn-icon" data-toggle="modal" data-target="#updateModal" wire:click.prevent='edit({{ $supplier->id }})'  ><i class="typcn typcn-edit"></i></button>
                                    @endcan
                                    @can('حذف المورد')
                                    <button class="btn btn-danger btn-icon" data-toggle="modal" data-target="#deleteModal" wire:click.prevent='delete({{ $supplier->id }})'><i class="typcn typcn-trash"></i></button>
                                    @endcan
                                    @can('حالة المورد')
                                    <button class="btn btn-info btn-icon" data-toggle="modal" data-target="#statusModel" wire:click.prevent='status({{ $supplier->id }})'><i class="fa fa-check"></i></button>
                                    @endcan
